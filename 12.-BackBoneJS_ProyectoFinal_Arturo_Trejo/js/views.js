...
var VistaListadoClientes = Backbone.View.extend({
	...
	render: function(){
		...
		this.collection.each(function(cliente){
			$("#listaClientes").append('<tr><td>' + cliente.get("id") + '</td><td>' + cliente.get("nombre") + '</td><td>' + cliente.get("apellido") + '</td><td>' + cliente.get("edad") + '</td><td>' + cliente.get("telefono") + '</td><td>' + cliente.get("email") +  '</td><td><a href="#mostrarCliente/' + cliente.get("id") + '" class="btn btn-primary btn-sm">Seleccionar</a></td><td><a href="#eliminarCliente/' + cliente.get("id") +'" class="btn btn-danger btn-sm" >Eliminar</a></td></tr>');
		},this);
